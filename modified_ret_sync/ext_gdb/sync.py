    ...
    def get_mod_by_addr(maps, addr):
        for mod in maps:
            if (addr > mod[0]) and (addr < mod[1]):
                module_name = mod[3].split(' ')[1]
        min_base = 0
        mod_sym = ''
        for mod in maps:
            if module_name in mod[3] and min_base == 0:
                min_base = mod[0]
                mod_sym = mod[3]
            elif module_name in mod[3] and min_base > mod[0]:
                min_base = mod[0]
                mod_sym = mod[3]
        if min_base > 0:
            return [min_base, mod_sym]
        return None
    ...
    def _invoke(self, arg, from_tty):
        ...
        for l in bt:
            ...
            if symbol == '??':
                ...
                if raddr in self.sync.symtable.keys():
                   continue
